...
This script reads the raw CSV produced by `fetch_prices.py` and applies all necessary cleaning and enrichment using pandas. It also joins the raw ticker list against `config/companies.py` to attach `company_name` and `sector`, since raw ingestion carries neither. It converts prices from cents to rands, handles any null values, standardises column names, and calculates three derived fields: daily return percentage, 7-day moving average, and 30-day moving average. The output is a clean, analytics-ready CSV saved to `data/clean/` with the same date stamp as the raw file it came from.
...
- **Why `fetch_prices.py` only outputs `ticker` and not `company_name`/`sector`.** Raw ingestion should only extract and land data exactly as the source returns it — no enrichment, no joins, no business context. `company_name` and `sector` don't come from yfinance; they come from `config/companies.py`, which makes attaching them a transformation step, not an extraction step. `clean_prices.py` is the single point where `ticker` gets resolved to `company_name`/`sector` via a join against the companies config, before cents are converted to rands and before the derived fields are calculated. This keeps enrichment in exactly one place instead of leaking into ingestion.
